...
- `history search` accepts source, literal query, merchant, date_from/date_to, limit (1–100), and offset. Results include source, record id, date, detail and evidence label, plus next_offset. Sources include calendar, receipts, lists, tasks, notes, routines, itineraries, milestones, decisions, special_events, messages, contacts, trips, pantry, gifts and project_expenses. Household, owner and explicit group-sharing boundaries remain enforced. Gift ideas follow the surprise rule: an agent never sees ideas saved for its own user by someone else, nor ideas another member kept private or shared with one other person. Archived routine entries are searchable. Deleted records cannot be recovered. Receipt notes contain scanned line items; checked lists and pantry inventory are not proof of purchase. Historical budget spending uses `budget get` with month; limits reflect current settings, not an original historical budget snapshot.
