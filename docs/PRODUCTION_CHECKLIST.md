...
- ✅ **Stripe live cutover 2026-08-26** — live products/prices + webhook `https://kinrows.com/api/subscription/stripe`; Railway on `sk_live_` / `pk_live_`; `STRIPE_ALLOW_TEST` deleted. Deploy `50fbf719` healthy (`/healthz`, catalog `"stripe":true`). Still do one real Checkout purchase before treating billing as battle-tested.
