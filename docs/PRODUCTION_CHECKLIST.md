...
- ✅ Railway env confirmed 2026-08-26 via CLI (project **Kinrows** / service **family-life-organizer**): `SESSION_SECRET` **rotated**, `ANTHROPIC_API_KEY`, APNs trio + `APNS_BUNDLE_ID` + `APNS_ENV=production`, `RESEND_API_KEY`, Stripe keys + webhook secret present. Leftover `GMAIL_USER` / `GMAIL_APP_PASSWORD` deleted (IMAP feature is gone).
...
- ⏳ **`STRIPE_ALLOW_TEST=1` still on** because production still has **test** Stripe keys (`sk_test_` / `pk_test_`). Do not unset until live keys + a live webhook secret are installed, or web Checkout webhooks will 400.
