...
- ✅ Rotating device-token auth, household authorization guards, parameterized SQL, money coercion, LIKE-injection escaping, cross-household tests, pre-launch isolation tests (`test/prelaunch-security.test.js`). `npm test` = 156 cases.
...
- ✅ **Stripe live cutover 2026-08-26** — live products/prices + webhook `https://kinrows.com/api/subscription/stripe`; Railway on `sk_live_` / `pk_live_`; `STRIPE_ALLOW_TEST` deleted. Deploy `50fbf719` healthy (`/healthz`, catalog `"stripe":true`). **Funnel (this branch):** confirmation polling, Resend billing emails, portal plan-switch, Lite 10 / Premium 40 chat caps with upgrade 429. Still do one real Checkout purchase after this branch deploys.
...
- ⏳ iOS unit/UI test target (backend `npm test` is 156 cases; the app has none).
